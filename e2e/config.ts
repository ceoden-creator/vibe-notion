...
export async function validateNotionBotEnvironment() {
	...
	const data = parseJSON<{ integration: { workspace_name?: string } }>(
		result.stdout,
	);
	if (!data?.integration?.workspace_name) {
		throw new Error(
			`Failed to parse auth status response. Got: ${result.stdout}`,
		);
	}
...
export async function validateNotionEnvironment(): Promise<string> {
	...
	if (wsResult.exitCode !== 0) {
		throw new Error(
			`Failed to list workspaces. Error: ${wsResult.stderr || wsResult.stdout}`,
		);
	}

	const workspaces = parseJSON<Array<{ id: string; name?: string }>>(
		wsResult.stdout,
	);
	if (!workspaces || workspaces.length === 0) {
		throw new Error(
			"No workspaces found. Please ensure your Notion account has at least one workspace.",
		);
	}

	const workspace = workspaces.find(
		(w) => w.name === NOTION_E2E_WORKSPACE_NAME,
	);
	if (!workspace) {
		throw new Error(
